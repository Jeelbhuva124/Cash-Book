...
export default function Subcategories() {
  ...
  const [formData, setFormData] = useState({ subcategory_name: '', category_id: '', active: true });
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        if (userCats.length > 0) {
          setFormData(prev => ({ ...prev, category_id: userCats[0].id }));
        }
      }
    } catch (err) {
  ...
  const loadSubcategories = async () => {
    try {
      const response = await fetch('http://localhost:5001/api/subcategory/select');
      const data = await response.json();
      if (data.success && data.data) {
        // Filter by user and active chalan_id
        const userSubs = data.data.filter(sub => 
          sub.user_email?.toLowerCase() === userEmail.toLowerCase() &&
          sub.chalan_id === selectedChalanId
        );
        setSubcategories(userSubs);
      }
    } catch (err) {
      console.error("Failed to load subcategories:", err);
      addToast("Failed to fetch subcategories from database", "error");
    }
  };
  ...
  const handleOpenModal = (sub = null) => {
    ...
    if (categories.length === 0) {
      addToast("Please create at least one category under this cashbook first", "warning");
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.subcategory_name.trim()) {
      addToast("Subcategory name is required", "warning");
      return;
    }
    if (!formData.category_id) {
      addToast("Please choose a category", "warning");
      return;
    }

    const matchedCat = categories.find(c => c.id === formData.category_id);
    const parentCategoryName = matchedCat ? matchedCat.category_name : '';

    const isEdit = !!editingSubcategoryId;
    const url = isEdit
      ? 'http://localhost:5001/api/subcategory/update'
      : 'http://localhost:5001/api/subcategory/insert';
    ...
    if (!isEdit) {
      ...
      payload.id = editingSubcategoryId;
    }
    ...
    try {
      ...
      if (data.success) {
        addToast(isEdit ? "Subcategory updated successfully" : "Subcategory added successfully", "success");
        handleCloseModal();
        loadSubcategories();
      } else {
        addToast(data.message || "Failed to save subcategory", "error");
      }
    } catch (err) {
      console.error("Save Subcategory Error:", err);
      addToast("Network error: failed to save subcategory", "error");
    }
  };
  ...
  const handleDelete = (id) => {
    setConfirmModal({
      ...
      onConfirm: async () => {
        try {
          const response = await fetch('http://localhost:5001/api/subcategory/delete', {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' },
          ...
          if (data.success) {
            ...
            addToast(data.message || "Failed to delete subcategory", "error");
          }
        } catch (err) {
          console.error("Delete Subcategory Error:", err);
          addToast("Failed to delete subcategory from database", "error");
        }
      }
    });
  };
  ...
  const sortedSubcategories = [...filteredSubcategories];
  if (sortConfig.key) {
    sortedSubcategories.sort((a, b) => {
      let valA, valB;
      if (sortConfig.key === 'no') {
        valA = subcategories.indexOf(a) + 1;
        valB = subcategories.indexOf(b) + 1;
      } else {
    ...
    <div className="p-6 md:p-8 w-full space-y-6 bg-transparent dark:bg-transparent min-h-screen text-foreground relative">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5 font-medium">
          Manage subcategories under main categories
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-card border border-border/80 rounded-2xl p-5 shadow-sm space-y-4">
        <div className="flex items-center gap-2 text-sm font-bold text-[#1e293b] dark:text-foreground">
          ...
          <span>Cashbook-wise Subcategories</span>
        </div>
        <p className="text-xs text-muted-foreground font-medium">
          Create subcategories mapping to specific categories
        </p>
      ...
      <div className="bg-white dark:bg-card border border-border/80 rounded-2xl p-6 shadow-sm space-y-4">
        <h2 className="font-bold text-sm text-foreground flex items-center gap-2">
          Subcategory List 
          <span className="text-muted-foreground font-normal">• {sortedSubcategories.length} Records</span>
        </h2>
          ...
          <table className="w-full text-left border-collapse table-fixed min-w-[1200px]">
            <thead>
              ...
              <tr className="bg-muted/30 border-b border-border text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
                ...
                <th className="px-4 py-3 w-[220px] cursor-pointer" onClick={() => handleSort('subcategory_name')}>
                  Subcategory Name {getSortIcon('subcategory_name')}
                </th>
                <th className="px-4 py-3 w-[180px] cursor-pointer" onClick={() => handleSort('category_name')}>
              ...
              <tr className="bg-muted/10 border-b border-border/40">
                ...
                <th className="px-4 py-2">
                  <input
                    ...
                    onChange={(e) => setFilters(prev => ({ ...prev, subcategory_name: e.target.value }))}
                    className="w-full px-2 py-1 text-xs border border-border bg-white dark:bg-card rounded-lg focus:outline-none focus:border-primary font-normal"
                  />
                </th>
                <th className="px-4 py-2">
            ...
            <tbody className="divide-y divide-border/60">
              {sortedSubcategories.map((sub, index) => (
                <tr key={sub.id} className="hover:bg-muted/30 transition-colors text-xs font-semibold text-foreground">
                  <td className="px-4 py-3.5 text-muted-foreground">{subcategories.indexOf(sub) + 1}</td>
                  <td className="px-4 py-3.5 text-foreground">{sub.subcategory_name}</td>
                  <td className="px-4 py-3.5 text-foreground">{sub.category_name}</td>
                  ...
                  <td className="px-4 py-3.5 text-muted-foreground truncate">{sub.created_by || 'Guest'}</td>
                  <td className="px-4 py-3.5 text-muted-foreground">{formatDateTime(sub.createdAt)}</td>
                  <td className="px-4 py-3.5 text-muted-foreground truncate">{sub.updated_by || 'Guest'}</td>
                  <td className="px-4 py-3.5 text-muted-foreground">{formatDateTime(sub.updatedAt)}</td>
          ...
          <div className="bg-white dark:bg-card border border-border rounded-2xl shadow-xl w-full max-w-[380px] relative z-10 animate-in fade-in zoom-in-95 duration-100 p-6 space-y-4 text-foreground">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide">Subcategory Name</label>
                <input 
                  type="text"
                  required
                  placeholder="e.g. Petrol, Chai Coffee"
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide">Parent Category</label>
                <Dropdown
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                />
                <label htmlFor="subcategory-active-status" className="text-xs font-semibold text-foreground cursor-pointer select-none">
                  Set as Active
                </label>
              </div>
              ...
              <div className="pt-2">
                ...
                >
                  {editingSubcategoryId ? 'Save Changes' : 'Create Subcategory'}
                </button>
              </div>
            </form>
          </div>
